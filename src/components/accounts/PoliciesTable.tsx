...
export const PoliciesTable = () => {
    ...
    const columns: DataTableColumn<PolicyItem>[] = [
        {
            ...
            label: 'LINE',
            render: (item) => (
                item.line && (<div className="flex items-center gap-2">
                    <div
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-[#8E8E8E] text-xs font-light" aria-label={`Policy ID: ${item.id}`}>{item.id}</span>
                    </div>
                </div>)
            ),
        },
        ...
        },
        {
            ...
            render: (item) => <span className="text-white font-light text-xs">{item.expiringPremium}</span>,
        },
        {
            key: 'renewalToTech',
        ...
        },
        {
            ...
            render: (item) => <span className="text-white font-light text-xs">{item.renewalPremium}</span>,
        },
        {
            key: 'rateChange',
            label: 'RATE CHANGE',
            render: (item) => (
                <span
                    className={`${item.status.label.startsWith('TOTAL')
                        ? 'text-white'
                        : (item.rateChange && parseFloat(item.rateChange) > 5 ? 'text-[#EF4444]' : 'text-white')
                        } font-light text-xs`}
                    aria-label={item.rateChange ? `Rate change: ${item.rateChange}` : 'Rate change not available'}
                >
                    {item.rateChange ?? 'N/A'}
                </span>
            ),
            ...
            render: (item) => (
                item.lossRatio.value !== null ? (
                    <div
                        ...
                        aria-label={`Loss ratio: ${item.lossRatio.value}%, ${item.lossRatio.color === 'green' ? 'good' :
                            item.lossRatio.color === 'yellow' ? 'moderate' : 'high'
                            }`}
                    >
                        {item.lossRatio.value}%
                    </div>
                ) : (
                    <div className="px-2 py-1 inline-flex items-center justify-center text-white font-light text-xs" aria-label="Loss ratio not available">
                        N/A
                    </div>
                )
            ),
    ...
    return (
        <div className="flex flex-col gap-6">
            <h2 className="text-white text-[32px] font-light mt-16">Policies</h2>
            <div className="bg-[#1e2233] rounded-[20px] p-6" role="region" aria-label="Policies table section">
